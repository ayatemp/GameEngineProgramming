...
bool InputSystem::Initialize()
{
    ...
    /*
	mController = SDL_GameControllerOpen(0);        //ここでゲームコントローラーをオープンしている（一つ目）
    ...
    */
    ...
    //課題８−１---------------------------------------------------------------------------------------
    //複数のコントローラーを読み取ってオープンする(使えるようにする)
    //std::cout<<"コントローラーの数:"<< SDL_NumJoysticks() <<std::endl;
    for(int i = 0 ; i < SDL_NumJoysticks(); i++)
    {
        if(SDL_IsGameController(i))
        {
            ...
            mControllerAllay.push_back(SDL_GameControllerOpen(i));
            //SDLゲームコントロラーポインタの配列に追加
            mState.ControllerAllay[i].mIsConnected = (mControllerAllay[i] != nullptr);
            memset(mState.ControllerAllay[i].mCurrButtons, 0,SDL_CONTROLLER_BUTTON_MAX);
            memset(mState.ControllerAllay[i].mPrevButtons, 0,SDL_CONTROLLER_BUTTON_MAX);
...
void InputSystem::PrepareForUpdate()
{
    ...
    for(int i=0; i<SDL_NumJoysticks(); i++)
    {
    memcpy(mState.ControllerAllay[i].mPrevButtons,
        mState.ControllerAllay[i].mCurrButtons,
        SDL_CONTROLLER_BUTTON_MAX);
    }
}
...
void InputSystem::Update()
{
    ...
    //課題８−１
    for(int num=0; num < SDL_NumJoysticks(); num++)
    {
        for (int i = 0; i < SDL_CONTROLLER_BUTTON_MAX; i++)
        {
            mState.ControllerAllay[num].mCurrButtons[i] =
                SDL_GameControllerGetButton(mControllerAllay[num],
                    SDL_GameControllerButton(i));
        }

        // Triggers
        mState.ControllerAllay[num].mLeftTrigger =
            Filter1D(SDL_GameControllerGetAxis(mControllerAllay[num],
                SDL_CONTROLLER_AXIS_TRIGGERLEFT));
        mState.ControllerAllay[num].mRightTrigger =
            Filter1D(SDL_GameControllerGetAxis(mControllerAllay[num],
                SDL_CONTROLLER_AXIS_TRIGGERRIGHT));

        // Sticks
        x = SDL_GameControllerGetAxis(mControllerAllay[num],
            SDL_CONTROLLER_AXIS_LEFTX);
        y = -SDL_GameControllerGetAxis(mControllerAllay[num],
            SDL_CONTROLLER_AXIS_LEFTY);
        mState.ControllerAllay[num].mLeftStick = Filter2D(x, y);

        x = SDL_GameControllerGetAxis(mControllerAllay[num],
            SDL_CONTROLLER_AXIS_RIGHTX);
        y = -SDL_GameControllerGetAxis(mControllerAllay[num],
            SDL_CONTROLLER_AXIS_RIGHTY);
        mState.ControllerAllay[num].mRightStick = Filter2D(x, y);
    }
}
