...
public:
    ...
    //課題８−２-------------------------------------------------
    //テキストファイルから文字を読み込んで連想配列に追加する関数
    void SetActionFile(std::string FileName);
    std::unordered_map<std::string,std::string> ButtonAllay;
    std::unordered_map<std::string,std::string> DeviceAllay;
