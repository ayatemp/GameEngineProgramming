...
void HUD::Draw(Shader* shader)
{
    ...
    for(Vector2& blip : mABlips)
    {
        DrawTexture_Angle(shader,mArrow, cRadarPos + blip, 0.5f, mArrowAngle);
    }
	// Radar arrow
...
void HUD::AddArrowComponent(ArrowComponent* ac)
{
    mArrowComps.emplace_back(ac);
}
void HUD::RemoveArrowComponent(ArrowComponent* ac)
{
    auto iter = std::find(mArrowComps.begin(), mArrowComps.end(),
        ac);
    mArrowComps.erase(iter);
}
...
void HUD::UpdateRadar(float deltaTime)
{
    ...
    mABlips.clear();
    for (auto ac : mArrowComps)
    {
        Vector3 targetPos = ac->GetOwner()->GetPosition();
        Vector2 actorPos2D(targetPos.y, targetPos.x);
        ...
        //課題１１−３
        mArrowDirection = playerPos2D - actorPos2D;
        Vector2 test = playerToTarget - mArrowDirection;
        mArrowAngle = Math::Atan2(test.x, test.y);
