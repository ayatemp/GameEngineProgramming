...
void Renderer::Draw()
{
	// 先にバックミラーテクスチャへ描画(ビューボードスケーリうは0.25)
	Draw3DScene(mMirrorBuffer, mMirrorView, mProjection, 0.25f);
	// 次に通常の３Dシーンをデフォルトのフレームバッファに描画
...
bool Renderer::CreateMirrorTarget()
{
	...
	{
		// もし問題があれば，フレームバッファを削除し
		// テクスチャを解放・削除して，falseを返す
		glDeleteFramebuffers(1, &mMirrorBuffer);
		mMirrorTexture->Unload();
		delete mMirrorTexture;
		mMirrorTexture = nullptr;
...
void Renderer::DrawFromGBuffer()
{
	...
	// Gバッファの深度バッファの内容をデフォルトのフレームバッファにコピー
	...
	// 深度テストを有効にするが深度バッファへの書き込みは無効にする
	glEnable(GL_DEPTH_TEST);
...
bool Renderer::LoadShaders()
{
    ...
    //課題１４−１
    mGGlobalShader->SetFloatUniform("uSpecPower", 100.0);
