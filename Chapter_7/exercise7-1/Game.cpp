...
void Game::GenerateOutput()
{
    ...
    if(xPosition < 400.0f) muki = 1;
    if(xPosition > 1000.0f) muki = -1;
    if(muki == 1)
    {
        xPosition += deltaTime;
        MoveActor->SetPosition(Vector3(xPosition, -75.0f, 0.0f));
    }
    else
    {
...
void Game::LoadData()
{
    ...
    MoveActor = new Actor(this);
	MoveActor->SetPosition(Vector3(500.0f, -75.0f, 0.0f));
	MoveActor->SetScale(1.0f);
	mc = new MeshComponent(MoveActor);
	mc->SetMesh(mRenderer->GetMesh("Assets/Sphere.gpmesh"));
	AudioComponent* ac = new AudioComponent(MoveActor);
